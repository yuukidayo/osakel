...
void main() async {
  ...
  debugPrint('ドリンクコレクションに簡易カテゴリID割り当てスクリプトを開始します...');
  ...
  try {
    ...
    for (var drinkDoc in drinksSnapshot.docs) {
      try {
        ...
        debugPrint('${drinkName ?? drinkDoc.id}: カテゴリID割り当て -> $selectedCategoryId');
        updatedCount++;
        operationCount++;
    ...
    for (var entry in categoryMap.entries) {
      ...
      debugPrint('  $categoryName: $count件 (ID: $categoryId)');
    }
    debugPrint('==================');
